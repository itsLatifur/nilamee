...
const Card = ({ imgSrc, title, startingBid, startTime, endTime, id }) => {
  ...
  return (
    <>
      <Link
        to={`/auction/item/${id}`}
        className="flex-grow basis-full bg-gradient-to-br from-burgundy-950/10 to-golden-950/5 dark:from-black/10 dark:to-gray-950/5 backdrop-blur-sm rounded-md group sm:basis-56 lg:basis-60 2xl:basis-80 border-2 border-golden-400 dark:border-golden-500 hover:border-burgundy-400 dark:hover:border-gray-600 transition-all duration-300 card-hover relative overflow-hidden"
      >
        ...
        <div className="px-2 pt-4 pb-2">
          <h5 className="font-semibold text-[18px] group-hover:text-golden-500 mb-2">
            {title}
          </h5>
          {startingBid && (
