    ...
    <>
      <section className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col min-h-screen py-4 justify-start">
        <div className="bg-gradient-to-br from-burgundy-950/20 to-golden-950/10 dark:from-black/20 dark:to-gray-950/10 whitestone:bg-white/30 whitestone:backdrop-blur-xl backdrop-blur-sm whitestone:backdrop-blur-xl mx-auto w-full h-auto px-2 flex flex-col gap-4 items-center py-4 justify-center rounded-md border-2 border-golden-400 whitestone:border-white/30 shadow-2xl">
          ...
          >
            <h3
              className={`text-golden-300 whitestone:text-gray-800 text-xl font-semibold mb-2 min-[480px]:text-xl md:text-2xl lg:text-3xl`}
            >
              Upload Payment Proof
            </h3>
            <div className="flex flex-col gap-2">
              <label className="text-[16px] text-golden-300 whitestone:text-gray-900">Amount</label>
              <input
                type="number"
                value={amount}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[16px] text-golden-300 whitestone:text-gray-900">Comment</label>
              <textarea
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                rows={7}
                className="text-[16px] py-2 bg-transparent border-[1px] rounded-md px-1 border-golden-400 whitestone:border-white/30 focus:outline-none text-warm-white focus:border-golden-300 transition-all"
              />
            </div>
            <button
              className="bg-gold-gradient mx-auto font-semibold text-xl transition-all duration-300 py-2 px-4 rounded-md my-4 border-2 border-golden-400 whitestone:border-white/30 shadow-lg text-warm-white btn-hover whitestone:text-white"
              type="submit"
            >
